...
public class IsNullStringQueryFactoryTest {
    ...
    public void testCorrectResults() throws IOException {
        Path indexPath = Paths.get(System.getProperty("java.io.tmpdir"), "jmh-lucene-indexes",
                IsNullStringQueryFactoryTest.class.getSimpleName());
        try (Directory dir = FSDirectory.open(indexPath)) {
            ...
            try (IndexReader reader = DirectoryReader.open(dir)) {
                ...
                for (IsNullStringQueryFactory factory : IsNullStringQueryFactory.values()) {
    ...
    public void testQ0CorrectQuery() {
        Query q = IsNullStringQueryFactory.Q0.create(FIELD_NAME);
        assertThat(q.toString(), is("+*:* -firstName:*"));
    }

    @Test
    public void testQ1CorrectQuery() {
        Query q = IsNullStringQueryFactory.Q1.create(FIELD_NAME);
        assertThat(q.toString(), is("+*:* -firstName:{* TO *}"));
    }

    @Test
    public void testQ2CorrectQuery() {
        Query q = IsNullStringQueryFactory.Q2.create(FIELD_NAME);
