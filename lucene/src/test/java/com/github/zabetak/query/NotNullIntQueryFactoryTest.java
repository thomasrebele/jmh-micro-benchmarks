...
public class NotNullIntQueryFactoryTest {
    ...
    public void testCorrectResults() throws IOException {
        Path indexPath = Paths.get(System.getProperty("java.io.tmpdir"), "jmh-lucene-indexes",
                NotNullIntQueryFactoryTest.class.getSimpleName());
        try (Directory dir = FSDirectory.open(indexPath)) {
            ...
            try (IndexReader reader = DirectoryReader.open(dir)) {
                ...
                for (NotNullIntQueryFactory factory : NotNullIntQueryFactory.values()) {
                    Query q = factory.create(FIELD_NAME);
                    TopDocs matches = searcher.search(q, 10);
                    assertThat(matches.scoreDocs.length, is(5));
                    ...
                    assertThat(matches.scoreDocs[4].doc, is(5));
                }
            }
        }
    }
    ...
    public void testQ0CorrectQuery() {
        Query q = NotNullIntQueryFactory.Q0.create(FIELD_NAME);
        assertThat(q.toString(), is("age:[-2147483648 TO 2147483647]"));
    }

    @Test
    public void testQ1CorrectQuery() {
        Query q = NotNullIntQueryFactory.Q1.create(FIELD_NAME);
        assertThat(q.toString(), is("age:[-2147483648 TO 2147483647]"));
    }

    @Test
    public void testQ2CorrectQuery() {
        Query q = NotNullIntQueryFactory.Q2.create(FIELD_NAME);
